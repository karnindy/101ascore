...
while ($row = oci_fetch_array($query,OCI_BOTH)) {
    if($row['SCORE_RANGE_DESC'] =='No. of Loans'){
        $table = $table."<tr style='background-color:#feccff;'>
                            ...
                            <td class='text-right'>" . number_format($row['ACTUAL_3M'], 0) . "</td>
                            <td class='text-right'>" . number_format($row['ACTUAL_12M'], 0) . "</td>
                            <td class='text-right'>" . number_format($row['ACTUAL_MORE12M'], 0) . "</td>
                            <td class='text-right'>" . number_format($row['REJECT_3M'], 2) . "</td>
                            <td class='text-right'>" . number_format($row['REJECT_12M'], 2) . "</td>
                            <td class='text-right'>" . number_format($row['REJECT_MORE12M'], 2) . "</td>														
                            <td class='text-right'>" . number_format($row['APPROVE_3M'], 2) . "</td>
                            <td class='text-right'>" . number_format($row['APPROVE_12M'], 2) . "</td>
                            <td class='text-right'>" . number_format($row['APPROVE_MORE12M'], 2) . "</td>
                            <td class='text-right'>" . number_format($row['OTHER_3M'], 2) . "</td>
                            <td class='text-right'>" . number_format($row['OTHER_12M'], 2) . "</td>
                            <td class='text-right'>" . number_format($row['OTHER_MORE12M'], 2) . "</td>
                        </tr>";
    } else if(preg_match("/Avg/i", $row['SCORE_RANGE_DESC']) || preg_match("/%/i", $row['SCORE_RANGE_DESC'])) {
        $table = $table."<tr>
                            <td>" . $row['SCORE_RANGE_DESC'] . "</td>
                            <td class='text-right'>" . number_format($row['ACTUAL_3M'], 2) . "</td>
                            <td class='text-right'>" . number_format($row['ACTUAL_12M'], 2) . "</td>
                            <td class='text-right'>" . number_format($row['ACTUAL_MORE12M'], 2) . "</td>
                            <td class='text-right'>" . number_format($row['REJECT_3M'], 2) . "</td>
                            <td class='text-right'>" . number_format($row['REJECT_12M'], 2) . "</td>
                            <td class='text-right'>" . number_format($row['REJECT_MORE12M'], 2) . "</td>							
                            <td class='text-right'>" . number_format($row['APPROVE_3M'], 2) . "</td>
