...
while ($row = oci_fetch_array($query,OCI_BOTH)) {
    if(strtolower($row['SCORE_RANGE_DESC']) == "total"){
        $table = $table."<tr style='background-color:#feccff;font-weight: 700;'>
                            ...
                            <td class='text-right'>" . (($row['APPROVE'] == "")? '' : number_format($row['APPROVE'], 0)) . "</td>
                            <td class='text-right'>" . (($row['REJECT'] == "")? '' : number_format($row['REJECT'], 0)) . "</td>
                            <td class='text-right'>" . (($row['TOTAL'] == "")? '' : number_format($row['TOTAL'], 0)) . "</td>
                            <td class='text-right'>" . (($row['PER_APPROVE'] == "")? '' : number_format($row['PER_APPROVE'], 2)) . "</td>
                        </tr>";
    } else if(preg_match("/Overrides %/i", $row['SCORE_RANGE_DESC']) || preg_match("/Rate/i", $row['SCORE_RANGE_DESC'])) {
        $table = $table."<tr>
                            <td>" . $row['SCORE_RANGE_DESC'] . "</td>
                            <td class='text-right'>" . (($row['APPROVE'] == "")? '' : number_format($row['APPROVE'], 2)) . "</td>
                            <td class='text-right'>" . (($row['REJECT'] == "")? '' : number_format($row['REJECT'], 2)) . "</td>
