...
group by reason_code_desc) b on substr(a.reason_code_desc_2,1,4) = b.reason_code_desc
...
and last_day(to_date('01'||'/'||'#MM#'||'/'||'#YYYY#','dd/mm/yyyy'))
group by reason_code_desc) c on substr(a.reason_code_desc_2,1,4) = c.reason_code_desc
left join (select reason_code_desc
,sum(nvl(pass_cutoff_reject,0)) pass_cutoff_reject
,sum(nvl(below_cutoff_approve,0)) below_cutoff_approve
,sum(nvl(approve_rpt1,0)+nvl(reject_rpt1,0)) total_actual
...
and last_day(to_date('01'||'/'||'#MM#'||'/'||'#YYYY#','dd/mm/yyyy'))
group by reason_code_desc) d on substr(a.reason_code_desc_2,1,4) = d.reason_code_desc) aa
inner join
