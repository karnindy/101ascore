...
while ($row = oci_fetch_array($query,OCI_BOTH)) {
    if(preg_match("/%/i", $row['CATEGORY'])){
        $table = $table."<tr>
                    <td>" . $row['CATEGORY'] . "</td>
                    <td class='text-right'>" . number_format($row['APP_IN_RANGE_3M'], 2) . "</td>
                    <td class='text-right'>" . number_format($row['APP_IN_RANGE_12M'], 2) . "</td>
                    <td class='text-right'>" . number_format($row['APP_IN_RANGE_MORE12M'], 2) . "</td>
                    <td class='text-right'>" . number_format($row['APPROVAL_3M'], 2) . "</td>
                    <td class='text-right'>" . number_format($row['APPROVAL_12M'], 2) . "</td>
                    <td class='text-right'>" . number_format($row['APPROVAL_MORE12M'], 2) . "</td>
                </tr>";
    } else {
        $table = $table."<tr>
                    <td>" . $row['CATEGORY'] . "</td>
                    <td class='text-right'>" . number_format($row['APP_IN_RANGE_3M'], 0) . "</td>
                    <td class='text-right'>" . number_format($row['APP_IN_RANGE_12M'], 0) . "</td>
                    <td class='text-right'>" . number_format($row['APP_IN_RANGE_MORE12M'], 0) . "</td>
                    <td class='text-right'>" . number_format($row['APPROVAL_3M'], 0) . "</td>
                    <td class='text-right'>" . number_format($row['APPROVAL_12M'], 0) . "</td>
                    <td class='text-right'>" . number_format($row['APPROVAL_MORE12M'], 0) . "</td>
                </tr>";
    }
