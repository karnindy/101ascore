...
,bb.actual
,bb.actual||'/'||case when nvl(bb.total_actual,0) = 0 then 1 else nvl(bb.total_actual,0) end * 100 per_actual

,bb.actual||'/'||case when nvl(bb.total_actual,0) = 0 then 1 else nvl(bb.total_actual,0) end * 100
||'/'|| case when (aa.dev/case when nvl(aa.total_dev,0) = 0 then 1 else nvl(aa.total_dev,0) end * 100) = 0 then 1
else (aa.dev/case when nvl(aa.total_dev,0) = 0 then 1 else nvl(aa.total_dev,0) end * 100) end / 100 change

,to_char(aa.score)point

,to_char((((bb.actual/case when nvl(bb.total_actual,0) = 0 then 1 else nvl(bb.total_actual,0) end * 100)
/ case when (aa.dev/case when nvl(aa.total_dev,0) = 0 then 1 else nvl(aa.total_dev,0) end * 100) = 0 then 1
else (aa.dev/case when nvl(aa.total_dev,0) = 0 then 1 else nvl(aa.total_dev,0) end * 100) end) / 100) * aa.score) point_diff
...
,to_char(round(sum(total.per_dev),5)) per_dev
,round(sum(total.actual),5) actual
,to_char(round(sum(total.per_actual),5)) per_actual
,to_char(round(sum(total.change),5)) change
,'' point
,'' point_diff
