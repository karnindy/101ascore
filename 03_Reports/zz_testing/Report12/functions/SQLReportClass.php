...
,'('||sum(a.delmore_90)||'/'||case when sum(a.active) = 0 then 0 else sum(a.active) end||') * 100' per_npls
,sum(a.ncb_amount) ncb_amount
,'('||sum(a.ncb_amount)||'/'||case when sum(a.current_balance) = 0 then 1 else sum(a.current_balance) end||') * 100' per_ncb_amount
...
,'('||sum(post.npls_account)||'/'||case when nvl(sum(post.active_account),0) = 0 then 0 else nvl(sum(post.active_account),0) end||')*100' per_cum_npl
from
(select aa.score_range_id
,nvl(npls_account,'0') npls_account
,nvl(active_account,0) active_account
