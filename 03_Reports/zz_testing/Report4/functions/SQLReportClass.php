...
,aa.score_range_desc
,to_char(nvl(approve,0)) approve
,nvl(reject,0) reject
,nvl(total,0) total
,to_char(nvl(per_approve,0)) per_approve
...
,sum(a.approve_rpt1)*100
||'/'
||case when max(total_approve) = 0 then 1 else max(total_approve) end per_approve
...
,to_char(sum(aa.approve)) approve
,sum(aa.reject) reject
,sum(aa.total) total
,to_char(round(sum(aa.per_approve),2)) per_approve
from
(select aa.score_range_id 
,aa.score_range_desc
...
,to_char(round(sum(below_cutoff_approve),2)) low_side_overides
,null
,null
,null
from prepare_source_st4
...
,(sum(a.below_cutoff_approve)*100)
||'/'
||(case when max(b.total_below_cutoff) = 0 then 1 else max(b.total_below_cutoff) end) per_low_side_overides
...
,to_char(round(sum(pass_cutoff_reject),2)) pass_cutoff_reject
,null
,null
,null
from prepare_source_st4
...
,(sum(a.pass_cutoff_reject)*100)
||'/'
||(case when max(b.total_pass_cutoff_reject) = 0 then 1 else max(b.total_pass_cutoff_reject) end) per_low_side_overides

,null
,null
,null
from prepare_source_st4 a inner join (select sum(nvl(pass_cutoff,0)) total_pass_cutoff_reject
from prepare_source_st4
where product_type = '#product_type#'
and model_name = '#model_name#'
and ('#card_type#' = 'รวมทุกประเภทบัตร' or card_type = '#card_type#')
...
,sum(a.pass_cutoff)||'*100'

||'/'
||(case when max(b.total_approval) = 0 then 1 else max(b.total_approval) end) per_total_approval
