...
,to_char('('||(sum(a.delmore_90)||'/'||case when sum(a.active) = 0 then 1 else sum(a.active) end) ||')* 100') per_npls
,sum(a.ncb_amount) ncb_amount
,to_char('('||(sum(a.ncb_amount)||'/'||case when sum(a.current_balance) = 0 then 1 else sum(a.current_balance) end) ||')* 100') per_ncb_amount
...
,to_char('('||(sum(post.npls_account)||'/'||case when nvl(sum(post.active_account),0) = 0 then 1 else nvl(sum(post.active_account),0) end)||')* 100')per_cum_npl
from
(select aa.score_range_id
,nvl(npls_account,0) npls_account
,nvl(active_account,0) active_account
...
,sum(a.delmore_90) npls_account
,to_char('('||(sum(a.delmore_90)||'/'||case when max(nvl(b.total_delmore_90,0)) = 0 then 1 else max(nvl(b.total_delmore_90,0)) end) ||')* 100') per_npls
,sum(a.ncb_amount) ncb_amount
,to_char('('||(sum(a.ncb_amount)||'/'||case when max(nvl(b.total_ncb_amount,0)) = 0 then 1 else max(nvl(b.total_ncb_amount,0)) end) ||')* 100') per_ncb_amount
from prepare_source_st11 a inner join (select sum(active) total_active
,sum(ncb_amount) total_ncb_amount
,sum(delmore_90) total_delmore_90
,sum(current_balance) current_balance                                        
...
,to_char('('||(total_1.npls_account||'/'||case when total_1.active_account = 0 then 1 else total_1.active_account end) ||')* 100') per_cum_npl
...
,sum(a.current_balance) active_amount
,sum(a.delmore_90) npls_account
,to_char('('||(sum(a.delmore_90)||'/'||case when sum(a.active) = 0 then 0 else sum(a.active) end) ||')* 100') per_npls
,sum(a.ncb_amount) ncb_amount
,to_char('('||(sum(a.ncb_amount)||'/'||case when sum(a.current_balance) = 0 then 0 else sum(a.current_balance) end) ||')* 100') per_ncb_amount
from prepare_source_st11 a inner join (select sum(active) total_active
,sum(ncb_amount) total_ncb_amount
,sum(delmore_90) total_delmore_90
,sum(current_balance) current_balance                                        
