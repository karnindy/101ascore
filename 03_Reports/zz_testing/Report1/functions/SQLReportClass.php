...
,'('||max(nvl(ccc.dev,0)) ||'/'|| case when max(nvl(ccc.total_dev,0)) = 0 then 0 else  max(nvl(ccc.total_dev,0)) end ||') * 100' per_dev

,sum(nvl(bbb.actual,0)) actual
,'('||sum(nvl(bbb.actual,0)) ||'/'|| case when sum(nvl(bbb.total_actual,0)) = 0 then 0 else  sum(nvl(bbb.total_actual,0)) end||') * 100' per_actual
...
(select case when sum(approve_rpt1) = 0 then 0 else sum(approve_rpt1) end total_approve
from prepare_source_st4
where product_type = '#product_type#'
and model_name = '#model_name#'
and ('#card_type#' = 'รวมทุกประเภทบัตร' or card_type = '#card_type#')
...
(select case when sum(reject_rpt1) = 0 then 0 else sum(reject_rpt1) end total_reject
from prepare_source_st4
where product_type = '#product_type#'
and model_name = '#model_name#'
and ('#card_type#' = 'รวมทุกประเภทบัตร' or card_type = '#card_type#')
