...
,to_char('('||actived_good||'/'||(case when bb.total_actived_good = 0 then 0 else bb.total_actived_good end)||')*100') per_cum_g_curr
from 
...
,to_char('('||actived_bad||'/'||(case when bb.total_actived_bad = 0 then 0 else bb.total_actived_bad end)||')*100') per_cum_b_curr
from 
(select a.score_range_desc
,count(*) actived_bad
from prepare_source_st8 a inner join prepare_source_st9 b
...
left join (select aa.score_range_desc
,to_char('('||actived_bad||'/'||(case when aa.total_actived = 0 then 0 else aa.total_actived end)||')*100') per_bad_rate_curr
from 
(select a.score_range_desc
...
,'('||aa.good||'/'||(case when bb.total_good = 0 then 0 else bb.total_good end) ||')* 100' per_good_dev
,'('||aa.bad||'/'||(case when bb.total_bad = 0 then 0 else bb.total_bad end) ||')* 100' per_bad_dev
,'('||(aa.good||'/'||(case when bb.total_good = 0 then 0 else bb.total_good end) ||')* 100') ||'-'||('('||aa.bad||'/'||(case when bb.total_bad = 0 then 1 else bb.total_bad end)||')* 100') sep_bg_dev
,'('||aa.bad||'/'||(case when aa.good+aa.bad = 0 then 0 else aa.good+aa.bad end) ||')* 100' per_bad_rate_dev
from
(
select a.score_range_desc
...
select 30
,'Total'
,to_char(round(sum(aaaa.per_cum_g_curr),0)) per_cum_g_curr
,to_char(round(sum(aaaa.per_cum_b_curr),0)) per_cum_b_curr
,to_char(round(sum(aaaa.sep_bg_curr),0)) sep_bg_curr
,'('||sum(aaaa.per_bad_rate_curr1)||'/'||sum(aaaa.per_bad_rate_curr2)||')'||'*'||100 per_bad_rate_curr
,to_char(round(sum(aaaa.per_good_dev),2)) per_good_dev
,to_char(round(sum(aaaa.per_bad_dev),2)) per_bad_dev
,to_char(round(sum(aaaa.sep_bg_dev),2)) sep_bg_dev
,'('||sum(aaaa.per_bad_rate_dev1)||'/'||sum(aaaa.per_bad_rate_dev2)||')'||'*'||100 per_bad_rate_curr
--,to_char(round(sum(aaaa.per_bad_rate_dev),2)) per_bad_rate_dev
from
(select curr.score_range_id
,curr.score_range_desc
,curr.per_cum_g_curr
...
and ('#branch_name#' = 'รวมทุกสาขา' or a.branch_name in ('#branch_name#'))
and extract(year from a.create_date) = '#YYYY#'
and extract(month from a.create_date) = '#MM#') bb on 1 = 1
) ddd on aaa.score_range_desc = ddd.score_range_desc
where aaa.flag = '#model_name#'
) curr 
left join 
(
