...
$table = "<table class='table table-bordered'>
            <thead style='background-color:#feccff;'>
                <tr>
                    ...
                    <th colspan='4' scope='col' class='text-center align-middle'>Currentt Validation Sample(%)</th>
...
while ($row = oci_fetch_array($query,OCI_BOTH)) {
    if(preg_match("/Total/i", $row['SCORE_RANGE_DESC'])){
        $table = $table."<tr style='background-color:#feccff;'>
                    ...
                    <td class='text-right'>" . $row['PER_CUM_G_CURR'] . "</td>
                    <td class='text-right'>" . $row['PER_CUM_B_CURR'] . "</td>
                    <td class='text-right'>" . $row['SEP_BG_CURR'] . "</td>
                    <td class='text-right'>" . $row['PER_BAD_RATE_CURR'] . "</td>
                    <td class='text-right'>" . $row['PER_GOOD_DEV'] . "</td>
                    <td class='text-right'>" . $row['PER_BAD_DEV']  . "</td>
                    <td class='text-right'>" . $row['SEP_BG_DEV'] . "</td>
                    <td class='text-right'>" . $row['PER_BAD_RATE_DEV'] . "</td>
                </tr>";
    } else {
        $table = $table."<tr>
        <td>" . $row['SCORE_RANGE_DESC'] . "</td>
